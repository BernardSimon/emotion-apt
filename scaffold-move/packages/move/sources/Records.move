module EmotionApt::records {
  ...
  // add record
  public fun add_record(account: &signer, keywords: vector<String>, description: String) acquires Records {
  ...
  // delete record
  public fun delete_record(account: &signer, index: u64) acquires Records {
    assert!(exists<Records>(signer::address_of(account)), NoUserRecords);
    let records = borrow_global_mut<Records>(signer::address_of(account));
